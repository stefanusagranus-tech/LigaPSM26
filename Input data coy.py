...
def load_data():
    try:
        ...
        return conn, df_personil, df_item, df_periode, df_sales_p
    except Exception:
        # Fallback reading via Pandas Direct URL
        ...
        return None, df_personil, df_item, df_periode, df_sales_p

conn, df_personil, df_item, df_periode, df_sales_p = load_data()
        ...
        if login_btn:
            if not username_input or not password_input:
                ...
                if not user_match.empty:
                    ...
                    # Password bisa NIK, 12345, atau nama kecil personil
                    ...
                    st.error("❌ Nama / NIK Personil tidak ditemukan dalam Database MASTER_PERSONIL.")
...
# ==========================================
    ...
    # Hitung total pencapaian penjualan personil saat ini
    user_sales = df_sales_p[df_sales_p["person_name"].str.upper() == person_name.upper()] if not df_sales_p.empty else pd.DataFrame()
    total_qty_personil = int(user_sales["actual_qty"].sum()) if not user_sales.empty else 0
    
    # Header Sapaan & Kata Motivasi Dinamis
    st.markdown(f"""
    <div class='card-neon'>
        <div>
    ...
    # Ringkasan Pencapaian Personil
    st.markdown("<h3 style='color: #38bdf8; font-size: 18px;'>📊 Ringkasan Penjualan Anda</h3>", unsafe_allow_html=True)
    ...
    with m1:
        ...
        <div class='card' style='text-align: center;'>
            ...
            <div class='metric-val'>{total_qty_personil:,} <span style='font-size:14px;'>Pcs</span></div>
        </div>
        """, unsafe_allow_html=True)
    with m2:
        total_transaksi = len(user_sales) if not user_sales.empty else 0
        st.markdown(f"""
        <div class='card' style='text-align: center;'>

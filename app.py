...
def save_database(sales_item_df, sales_person_df):
    # Menyimpan kembali perubahan data transaksi ke file Excel tanpa merusak master sheet
...
if selected_tab == "01 · Overview":
    ...
    c1.metric("Target Toko", f"{tot_target:,.0f} Pcs")
    c2.metric("Actual Sales", f"{tot_actual:,.0f} Pcs")
    c3.metric("Achievement", f"{ach:.1f}%")
    c4.metric("Gap Target", f"{gap:,.0f} Pcs")
...
# --- TAB 02: DETAIL ITEM ---
    ...
    if not si_df.empty:
        top_item = si_df.loc[si_df["ach"].idxmax()]
        bad_item = si_df.loc[si_df["ach"].idxmin()]
...
# --- TAB 04: PENCAPAIAN PERNIK ---
    ...
    st.write(f"Rincian penjualan item pernik oleh staf: **{selected_person}**")
...
# --- TAB 06: INPUT & RESET DATA (KHUSUS EDITOR) ---
    ...
    with tab_in1:
        ...
        # Item yang muncul hanya yang terdaftar di SALES_ITEM pada periode tersebut
        ...
        if item_dict:
            ...
            with col_b1:
                if st.button("💾 Simpan Sales Personil", use_container_width=True):
                    ...
                    st.success("Data sales personil berhasil disimpan ke database Excel!")
            with col_b2:
                if st.button("🗑️ Hapus Data Personil (Berdasarkan Staf & Item)", use_container_width=True):
                    ...
                    save_database(st.session_state.sales_item_df, st.session_state.sales_person_df)
                    st.warning("Data personil berhasil dihapus dari Excel!")
        else:
        ...
        if item_dict_t:
            ...
            with col_t1:
                if st.button("📦 Update Actual Toko", use_container_width=True):
                    ...
                    save_database(st.session_state.sales_item_df, st.session_state.sales_person_df)
                    st.success("Aktual item toko berhasil diperbarui di Excel!")
            with col_t2:

...
# --- TAB 03: PENJUALAN PERSONIL ---
    ...
    # 1. AMBIL DATA DARI GOOGLE SHEETS (si_df)
    if "si_df" in st.session_state and not st.session_state.si_df.empty:
        sp_df = st.session_state.si_df.copy()

        # Filter berdasarkan periode jika ada
        if selected_period_id and "period_id" in sp_df.columns:
            sp_df = sp_df[sp_df["period_id"] == selected_period_id]

        # Pastikan kolom actual_qty bertipe numerik
        if "actual_qty" in sp_df.columns:
            sp_df["actual_qty"] = pd.to_numeric(
                sp_df["actual_qty"], errors="coerce"
            ).fillna(0)

        # Cek apakah kolom person_name ada di dalam data
        if "person_name" in sp_df.columns and not sp_df.empty:

            # 2. AGRAGASI DATA PER PERSONIL
            summary_person = (
            ...
            with col_table:
                ...
                for _, row in summary_person.iterrows():
                    ...
                    <tr style="border-bottom: 1px solid #1e293b;">
                        ...
                        <td style="padding: 12px; color: #00ff88; font-weight: bold;">{row['actual_qty']:,.0f} Pcs</td>
            ...
            st.warning(
                "⚠️ Kolom 'person_name' tidak ditemukan atau data personil kosong."
            )
    else:
        st.warning("⚠️ Data Google Sheets (`si_df`) belum terload.")

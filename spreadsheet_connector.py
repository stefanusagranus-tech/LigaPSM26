...
def render_debug_panel():
    ...
    with st.sidebar:
        ...
        if st.button(
            ...
            _tahun_int = _now.year
            _bulan_str = _now.strftime("%B").upper()
            _sheet_name = f"{_bulan_str} {_tahun_int}_PSM"
            
            st.sidebar.info(f"⏳ Isi kolom di sheet `{_sheet_name}`...")
            
            _ok, _msg, _n = isi_laporan_psm(_bulan_int, _tahun_int, _sheet_name)
        ...
        st.markdown("---")
        st.markdown("### 📊 Test Isi Laporan SUEGER")

        if st.button(
            "📊 ISI Kolom SUEGER (Bulan Ini)",
            key="btn_isi_sueger",
            use_container_width=True,
        ):
            from datetime import datetime
            ...
            _tahun_int = _now.year
            _bulan_str = _now.strftime("%B").upper()
            _sheet_name = f"{_bulan_str} {_tahun_int}_SUEGER"
            
            st.sidebar.info(f"⏳ Isi kolom di sheet `{_sheet_name}`...")
            
            _ok, _msg, _n = isi_laporan_sueger(_bulan_int, _tahun_int, _sheet_name)

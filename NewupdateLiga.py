...
# --- TAB 06: INPUT & RESET DATA ---
  ...
  with tab_in1:
    ...
    if is_visitor:
      ...
      filtered_si_m = (
          si_df[si_df["period_id"] == m_p_id]
          if not si_df.empty and "period_id" in si_df.columns
          else pd.DataFrame()
      )
      items_list = (
          filtered_si_m[["item_id", "item_name"]].drop_duplicates().to_dict("records")
          if not filtered_si_m.empty
          else []
      )

      if not items_list:
        st.warning(
            "⚠️ Tidak ada daftar item produk yang terdaftar pada periode"
            f" **{m_period_name}**."
        )
      else:
        st.markdown("---")
    ...
    if not is_admin:
      ...
      if sp_del_sub.empty:
        ...
        if mode_hapus == "Hapus Item Tertentu Saja":
          ...
          st.error(
              f"⚠️ Perhatian: Aksi ini akan menghapus SELURUH catatan penjualan"
              f" {d_person} pada periode ini."
          )
          if st.button(

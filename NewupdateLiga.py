...
# --- TAB 06: INPUT & RESET DATA ---
        ...
        if item_dict:
            ...
            with cb1:
                if st.button("💾 Simpan Data Personil", use_container_width=True):
                    ...
                    st.success("✅ Data berhasil disimpan secara permanen ke Google Sheets!")
                    st.rerun()
            ...
            with cb2:
                if st.button("🗑️ Hapus Transaksi Staf", use_container_width=True):
                    ...
                    sync_store_sales_from_personnel()
                    save_database(st.session_state.sales_item_df, st.session_state.sales_person_df)
                    st.warning("⚠️ Transaksi berhasil dihapus dan diperbarui di Google Sheets!")
                    st.rerun()
        else:
    ...
    with tab_in2:
        ...
        if not is_admin:
            st.warning("🔒 Sub-menu ini khusus untuk Admin.")
        else:
            ...
            if item_dict_t:
                ...
                with ct1:
                    if st.button("📦 Update Total Toko Manual", use_container_width=True):
                        ...
                        save_database(st.session_state.sales_item_df, st.session_state.sales_person_df)
                        st.success("Aktual produk toko diperbarui!")
                        st.rerun()

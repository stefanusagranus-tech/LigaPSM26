...
if "portal_prep_ready" in st.session_state and st.session_state.portal_prep_ready:
    ...
    # =========================================================================
        ...
        # =========================================================================
        # 🚪 KONDISI 1: MEJA RESEPSIONIS UTAMA (MASTER INVESTIGATION ENGINE)
        # =========================================================================
        # 🔍 KOTAK DETEKTIF: Menampilkan isi memori asli agar kita tahu apa yang salah
        with st.expander("🔍 LAPORAN INTEL (KLIK UNTUK LIHAT MEMORI APLIKASI)", expanded=True):
            st.write("Jika menu di bawah ini tidak sesuai, pantesan kartunya mogok jalan:")
            st.json({
                "current_camp_menu": st.session_state.get("current_camp_menu", "KOSONG"),
                "campaign_sub_page": st.session_state.get("campaign_sub_page", "KOSONG")
            })
        ...
        with col_lobby1:
            with st.container(border=True):
                ...
                if st.button("Lihat Status ➔", use_container_width=True, key="btn_lobby_buruan_master_test"):
        ...
        with col_lobby2:
            with st.container(border=True):
                ...
                if st.button("Ambil Quest ➔", use_container_width=True, key="btn_lobby_misi_master_test"):
        ...
        st.markdown("<div class='rpg-back-btn-box'>", unsafe_allow_html=True)
        if st.button("⬅️ KEMBALI KE KEMAH PERSIAPAN", use_container_width=True, key="btn_exit_master_test"):
            st.session_state.current_camp_menu = "main"
            st.rerun()
        st.markdown("</div>", unsafe_allow_html=True)
